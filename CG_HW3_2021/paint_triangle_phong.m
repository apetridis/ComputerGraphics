...
% verts_n: 3 Normal vectors of Triangle Vertices (3x3)
% 
function Y = paint_triangle_phong(img, vertices_2d, vertex_colors, verts_n, I_a, bcoords,cam_pos,mat,lights)
            ...
            if size(boundary_points) > [1 1]
             ...
             % Find normal on first boundary
                n_left = vector_interp( ...
                    ...
                    verts_n(active_edges(1),:) , ...                % Normal of Vertex 1
                    verts_n(next(active_edges(1),3),:) , ...        % Normal of Vertex 2
                    2 ...                                           % Vertical Interpolation
                );
                
                % Find normal on second boundary
                ...
                for i=1:size(norm_line,1)
                    color(i,:) = ambient_light(mat,colors(i,:)',I_a)' + diffuse_light(bcoords, norm_line(i,:)',[0;0;0],mat,lights)' + specular_light(bcoords,norm_line(i,:)',[0;0;0],cam_pos,mat,lights)';
                end
                img( y , round( boundary_points(1,1) ) : round( boundary_points(2,1) ), :) = color(:,:);                     
            end
    end       
